...
inline size_t HashMap<K, M, H>::size() const {
    return const_cast<HashMap<K, M, H>*>(this)->size();
}
...
bool HashMap<K, M, H>::contains(const K& key) const {
    return const_cast<HashMap<K, M, H>*>(this)->contains(key);
}
...
HashMap<K, M, H>& HashMap<K, M, H>::operator=(const HashMap<K, M, H>& other) {
    if (this != &other) {
        *this = std::move(HashMap<K, M, H>(other));
        // this->clear();
        // this->_hash_function = other._hash_function;
        // this->_buckets_array.resize(other.bucket_count(), nullptr);
